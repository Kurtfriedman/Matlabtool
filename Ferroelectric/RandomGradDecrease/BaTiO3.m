...
Px0=normrnd(1e-13,flucstd,[scale,scale]);
Py0=normrnd(1e-13,flucstd,[scale,scale]);
...
for i=1:iter_num
    x=randi([1 128]);y=randi([1 128]);
    %%-----------------derivative of Ginzburg-Landau energy-----------------%%
    dG_GL_k_Px2_alpha1=2.*alpha1.*Px2(x,y);
    ...
    temp1=kx(x,y).*k_Px2(x,y)+ky(x,y).*k_Py2(x,y);
    dG_k_es_Px2=temp1.*kx(x,y)./elesta_kernal(x,y);
    dG_k_es_Py2=temp1.*ky(x,y)./elesta_kernal(x,y);
    dG_k_es_Px2(1,1)=0;
    dG_k_es_Py2(1,1)=0;
    ...
    kernal_sum1=real(fftshift(ifft2((A11(x,y).*Y1_k(x,y)+A12(x,y).*Y2_k(x,y)+A16(x,y).*Y6_k(x,y))/d)));
    kernal_sum2=real(fftshift(ifft2((A21(x,y).*Y1_k(x,y)+A22(x,y).*Y2_k(x,y)+A26(x,y).*Y6_k(x,y))/d)));
    kernal_sum6=real(fftshift(ifft2((A61(x,y).*Y1_k(x,y)+A62(x,y).*Y2_k(x,y)+A66(x,y).*Y6_k(x,y))/d)));
