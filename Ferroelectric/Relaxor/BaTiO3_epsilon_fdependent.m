...
while var>terminal
    ...
    varPx=Px3-Px2; varPy=Py3-Py2;
    var=max(max(sqrt(abs(varPx).^2+abs(varPy).^2)));
    terminal=(10^floor(log10(mean2(sqrt(Px2.^2+Py2.^2)))))*5e-4;
...
Tmax=400;%generate temperature gradient
...
f=20000;relaxtime3=round(1/(deltat*f*sampling));%set polarize iteration parameter
Px_mean=zeros(length(T_series),length(acE));
domain_history=cell(length(T_series),2);
for T=T_series
%%-------------heating--------------%%
    l=l+1;var=1; terminal=0.5;
    step=0;
    while var>terminal
        %%-----------------derivative of Ginzburg-Landau energy-----------------%%
...
% in=1;
in2=60;
save('S_298K_c04_E1e6f2e4epsilon.mat','Px_mean','T_series','domain_history');
% subplot(1,2,1)
% quiver(x,y,domain_history{in,1},domain_history{in,2})
% subplot(1,2,2)
% colorquiver(x,y,domain_history{in2,1},domain_history{in2,2})
% color_quiver(Px,Py,angle,scale,color,arrowdensity,linewidth,rescaling_factor)
